...
# This project was completed in one of my public health classes. The data set was provided by the instructor and was cleaned in Excel.The data set contains information on the
...
# Creating factor variables rx_f and status_f from the character variables rx and status
...
# Obtaining relative frequency tables (i.e., proportions) of rx_f and status_f
...
# Obtaining relative frequency tables (with row proportion) of rx_f and died_f 
prop.table(table(Prostate$rx_f, Prostate$died_f), 1)


# Obtaining relative frequency tables (with column proportion) of rx_f and died_f.
...
# Examining correlations- Using the subset function to create a data frame with only the continuous variables of interest from the Prostrate data set
